...
switch($_GET['act']) {
	case 'send':
		...
		if(!$from_uxh) die('你还没有登录!');
		if(!isset($_POST['u']) || !isset($_POST['t']) || !isset($_POST['c']) || !isValidXH($_POST['u'])) die('参数错误');
		$to_uxh = $_POST['u'];
		$title = addslashes(htmlspecialchars($_POST['t']));
		$content = addslashes(htmlspecialchars($_POST['c']));
		if(empty($title)||empty($content)) die('标题或内容为空');
		if(mb_strlen($title) > 64) die('标题过长（大于64个字符）');
		if(mb_strlen($content) > 500) die('内容过长（大于500个字符）');
